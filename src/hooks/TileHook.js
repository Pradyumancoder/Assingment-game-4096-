...
// The useEventListener hook is used to listen for key events and trigger the corresponding move
...
const randomId = () => Math.random().toString(32).substr(3, 8);
// rotateMatrix function rotates the board by 90 degrees, which is used to simplify the logic of the move functions
...
// clearTileState function creates a copy of the board with all
//  the tile states (isNew, isMerge) reset, which is used to prepare the board for the next move
...
// generateRandomTile function adds a new tile with a random value to the board,
// which is used to create a new tile after each move
const generateRandomTile = ({ tilesArr, addScore = 0 }) => {
  const noValueTiles = tilesArr
    .concatAll()
    .filter((tileData) => !tileData.value);
  const choosedTile =
...
// rePositionTiles function updates the x and y coordinates of each tile,
// which is used to simplify the rendering of the board.
...
// The useTiles hook is the main part of the code that manages the state of the game.
const useTiles = (size = 8) => {
  ...
  // gameCondition function is used to check whether the game
  // is over (WIN or LOSS), based on the current state of the board
  const gameCondition = () => {
    const tileArr = tiles
      .concatAll()
    ...
    if (tileArr.length === size * size) {
      ...
      // Essentially, this code is checking if any two adjacent tiles have the
      //  same value property, and if so, it sets the game status to PLAYING
  ...
  // handleKeyDown function is called whenever the player presses an arrow key, and it
  //  updates the game state by calling one of four move functions (moveLeft, moveRight, moveUp, or moveDown), depending on the direction of the key press
  const handleKeyDown = () => {
    switch (key) {
      case 37: {
        moveLeft();
  ...
  // The initialTiles function is called in the useEffect hook, which runs after the component mounts
  const initialTiles = () => {
    ...
    generateRandomTile({ tilesArr }).then(generateRandomTile).then(saveTile);
